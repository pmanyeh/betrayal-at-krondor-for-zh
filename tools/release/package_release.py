...
本版主要新增功能
----------------
・訊息紀錄：在正常 3D 探索時按 L，或從遊戲中選項選擇「訊息紀錄」，
  可回看對話、玩家回答與劇情。支援鍵盤及滑鼠操作，長文可分頁閱讀。
・快速存讀檔：正常 3D 探索時按 F5 快速存檔，按 F9 讀取最新的有效快存。
・常駐效果資訊：點擊 3D 探索畫面上方的效果圖示，可查看法術作用與
  剩餘遊戲時間；資訊卡不會暫停操作，三秒後自動消失。
・滑鼠視角模式：
  在 3D 探索畫面按 F2 可切換（預設關閉）：滑鼠控制視角，W／S 前後
  移動，A／D 左右平移，E 或滑鼠左鍵啟用準星指向的物件；按住 Ctrl
  可暫時恢復游標並操作介面。此模式保留原版 3D 景窗與完整 UI，不會
  把 3D 畫面放大至全螢幕。開啟時會自動解除循路前進，且在此模式下
  不能再次啟用循路前進。

訊息紀錄會以與存檔同名的 .MLG 檔保存。備份、複製或搬移存檔時，請把
.GAM 與 .MLG 一起處理；舊存檔沒有 .MLG 仍可正常遊玩，但無法補回先前
的對話。完整功能與操作方式請參閱下列兩份說明。

隨附說明
--------
「中文按鍵說明.txt」整理遊戲各畫面的常用鍵盤操作。
「新增功能說明.txt」整理官方 1.02 非光碟修補，以及本中文化額外加入的
訊息紀錄、操作、自動存檔、快速存讀檔、地圖和介面功能。
...
def write_readme(release_dir: Path) -> None:
    data = README_TEXT.replace("\r\n", "\n").replace("\n", "\r\n").encode("utf-8-sig")
    (release_dir / "README_安裝說明.txt").write_bytes(data)

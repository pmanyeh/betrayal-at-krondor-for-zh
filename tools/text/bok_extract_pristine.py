#!/usr/bin/env python3
"""Extract canonical Cxx.BOK chapter-book files from Krondor's resource archive."""
...
def extract_pristine_bok(rmf_path: Path, archive_path: Path, output_dir: Path) -> list[Path]:
    """Copy every original Cxx.BOK from an RMF-indexed archive.
    ...
    with archive_path.open("rb") as archive:
        for index in range(count):
            ...
            upper = name.upper()
            if not (upper.startswith("C") and upper.endswith(".BOK")):
                continue
    ...
    if not extracted:
        raise ValueError("no Cxx.BOK entries found in the resource archive")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Extract all original Cxx.BOK files without modifying the game installation."
    )
    ...
    parser.add_argument("--output-dir", type=Path, default=repo_root / "scratchpad" / "pristine_bok")
    args = parser.parse_args()
    try:
        paths = extract_pristine_bok(args.rmf, args.archive, args.output_dir)
    except (OSError, UnicodeDecodeError, ValueError) as exc:
        print(f"ERROR: {exc}", file=sys.stderr)
        raise SystemExit(1) from exc
    print(f"Verified/extracted {len(paths)} pristine BOK file(s) to {args.output_dir}")

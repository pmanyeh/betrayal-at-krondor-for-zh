...
- [ ] 3D 場景不再出現首次進場或滑鼠左右鍵文字提示。
- [ ] 3D 場景按住 `Tab` 顯示所有目前有效角框，放開後無殘影。
- [ ] 移動或轉向後，角框位置與新的場景命中區同步更新。
- [ ] 靠近大型或畫面邊緣物件時，框線不超出 3D viewport、不蓋住下方 UI。
- [ ] 城鎮／室內按住 `Tab` 只框人物與物件，不框說明文字區。
- [ ] 按住 `Tab` 時滑鼠游標不會被通用選單導覽移走。
- [ ] 點擊、右鍵查看、說明翻頁、離開場景後都不殘留角框。
- [ ] 快速反覆按放 `Tab` 不閃退，滑鼠游標背景能正常還原。

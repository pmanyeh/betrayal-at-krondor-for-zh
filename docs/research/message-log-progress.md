...
- **內容頁直接切換紀錄**：在內容頁（Detail View）中支援直接以 Up/Down 鍵切換至前一則／下一則訊息紀錄，毋需每次返回清單頁。
- 重編產物：`KRONDOR.EXE` 491744 bytes，SHA-256 `f05cd8165a41ac0f088dca0f75feabc07cca74841f06be5c6f82faffed938326`（含 F2 提示編碼修復）；已部署至 `dist/test_v100_zh/krondor.exe`，備份為 `scratchpad/KRONDOR_msglog_phase4_v4.EXE`。
...
### 閱讀器預覽縮排與滑鼠操作（2026-09-07）

- 列表摘要正文由 x=20 改為 x=52，固定向右縮排兩個 16px 中文字寬度；可用寬度同步由 280 改為 248，不超出原本右界。
- 閱讀器等待輸入時會持續刷新軟體游標，不再因只等待 `kbhit_read()` 而看似凍結。進入時將游標置於 `(160,100)`，避免從 3D 滑鼠視角或選單邊界帶入不可見位置。
- 滑鼠左鍵可在列表點選該列並開啟內容；內容頁左／右半邊分別切換前／後頁；底部說明列可返回。右鍵在內容頁返回列表、在列表離開閱讀器。鍵盤操作全部保留。

...
- `F2`：切換滑鼠視角模式；每次啟動遊戲時預設關閉，本次遊戲期間保留狀態。開啟後仍保留經典 294×101 3D 景窗與完整 UI。若循路前進已開啟，進入滑鼠視角時會直接解除；滑鼠視角期間循路按鈕維持停用，`R`／循路 action 也不會重新啟用，關閉滑鼠視角後再依目前道路狀態恢復。
- 滑鼠左右移動：水平旋轉視角；滑鼠向上抬頭、向下低頭，限制在基準視角上下各約 11.25°。
- `W`／`S`：前進／後退。
- `A`／`D`：向左／向右平移；方向鍵仍保留左轉／右轉。
- `Q`：保留向左平移；`E` 改為啟用準星指向的物件。
- 滑鼠左鍵：在捕捉狀態下同樣啟用準星指向的物件。
- 按住 `Ctrl`：暫停視角控制、隱藏準星並恢復原先游標位置，可直接操作完整 UI；放開後重新捕捉滑鼠。
- 按住 `Tab`：繼續顯示全部有效熱區；準星目前指向的物件以黃色框線標示。

準星使用經典 3D viewport 的中心。無目標時是灰色，有目標時轉為黃色。模式切換會顯示兩秒中文提示。
...
2. 先開啟循路前進再按 `F2`，確認循路立即解除且 WASD 可正常移動；滑鼠視角期間按 `R` 或點循路按鈕均不能啟用，關閉 `F2` 後才恢復。
3. 滑鼠左右移動方向正確、沒有自行漂移；移動速度不會過快或過慢。
4. `W/S/A/D` 分別前進、後退、左平移、右平移；方向鍵仍能前後移動及左右轉向；`Q` 仍可左移。
5. 分別對人物、門、寶箱／袋子、地上物品或屍體測試：準星進入熱區時變黃並框出單一目標；`E` 與左鍵各只觸發一次正確互動。
6. 準星對著物件時按住 `Tab`，應同時看見全部白色熱區框與目前的黃色目標框；放開後只留下目前目標。
7. 按住 `Ctrl` 後游標應恢復且可以點擊 UI；放開後游標消失、準星恢復且視角不跳動。
8. 依序開啟並離開施法、近距離地圖、世界地圖、紮營、遊戲選單、角色資訊與物品欄，確認各畫面游標正常，返回探索後自動恢復滑鼠視角。
9. 走入劇情對話及用 E 觸發人物對話，確認對話開始時游標出現、不需按 Ctrl 即可翻頁或點選選項；對話結束回到探索後自動恢復滑鼠視角。
10. 進入城鎮並操作其中的商店、旅店或神殿；另測試地面物品、遭遇／戰鬥及區域切換，確認各畫面游標存在，返回探索後自動恢復滑鼠視角。
11. 關閉模式後，A/D 左右轉、Q/E 左右平移、滑鼠點物件與 Tab 全熱區框均維持原行為。
12. 順帶確認 F5／F9、C、M、F、T、O、B 與 1／2／3 等既有熱鍵沒有迴歸。
13. 確認 FPS 模式始終保留經典 3D 景窗、邊框、人物、按鈕、caption 與羅盤，畫面無全螢幕放大、殘影或破面。

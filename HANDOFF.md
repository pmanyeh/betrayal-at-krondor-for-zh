...
**最後更新：** 2026-09-04（本輪遊戲體驗改造均已由使用者實機驗收：**按住 Tab 顯示可互動熱區**、**法術分類顯示已學數量／空分類顯示「尚未學習」**、**10 槽輪替自動存檔**、**地圖顯示已發現洞穴入口記號**、**俯瞰地圖支援 WASDQE 與 T 紮營**。以物件中心為基準、隨地圖視野縮放且持久留存的範圍菱形已確認正常；地標目錄已依十章切換、傳送、對話、GDS 場景與 WLD 資料完整交叉稽核，目前只保留世界地圖未標示的 8 個洞穴／礦坑／地城地表據點，不再替一般城鎮重複加菱形。新版另會將三種察覺法術實際顯示的所有結果持久顯示為俯瞰地圖紅點；紮營畫面三個英文圖片按鈕也已改為 10×10 中文小字，並依實機回報將一般狀態改為較亮、按下狀態改為較暗；已完成重編與離線驗證，待複驗配色。完整紀錄見 [`docs/research/ux-improvements-2026-09-02.md`](docs/research/ux-improvements-2026-09-02.md)。）
...
- **發布包不再內附新手存檔**（2026-09-04）：密技中心已能讓玩家自行取得金幣、物品、調整能力與開放傳送點，因此移除 `GAMES/Plus.G01/`、其兩份基準存檔與 `build_starter_save.py`；`package_release.py` 也不再建立或白名單放行任何存檔。發布包的 `game_data/` 現在只允許一份放置說明，確保不會夾帶存檔或原版遊戲資料。重建後 ZIP 內 `.GAM` 檔案數為 0，SHA-256 `1b02d5de413f00ce3213e378cc9343e98ca20e7bbe76a185f79bbd88dc6ac95e`。
...
- **密技中心金錢按鈕已由「取得 5000 金幣」修正為「取得 500 金幣」**（2026-09-04）：`TOWNSCN.C` 的 `g_gameState.nParty_gold += 5000` 使用十分之一金幣的內部單位，顯示時經 `gstate_format_money()` 除以 10，實際增加量是 500 金幣，與原文 `Get: 500 Gold` 一致；因此保留原版程式行為，只修正 `MENUPAGE.json` 的誤譯與說明。重建後 `REQ_KNOC.DAT` SHA-256 `902622c45a6f472ad936cb7dd757166f0699a7e5175c56e743d157f21a2250a0`，已部署至測試版及發布包；100 項測試通過。最新發布 zip SHA-256 `e30225c41b8edb26aacc4aa5c141366e1115c5c6c01004cda3ba0f22bbda3191`，內含 0 個 `.GAM`。
- **神殿傳送介面中文排版已修整並實機驗收定案**（2026-09-04）：採不縮放 Logo 的安全方案，將 `From:`／`To:` 移至各自傳送 Logo 左側，兩個 16×16 中文地名移到 Logo 上方並在首輪後再一致下移 3 pixels，避免第一個地名壓到 `Teleport:` 標題，同時不再與 Logo 重疊。費用列僅在繪製動態「數字＋金幣」時啟用 8×16 倚天 ASCII 混排，使數字與 16×16 中文同高。引擎提交 `2a90a79`／位置微調 `5fb5ff7`；Borland C++ 3.1 重編成功，`KRONDOR.EXE` **470624 bytes**、SHA-256 `ccf685cb434c91e168d6228595c24e0527699b4b560e443cb424c1054f7cf38b`，兩個 OVL byte-identical，100 項測試通過。正式 bspatch **55326 bytes**（SHA-256 `b262b89c2d38822d6af242d36b21682cfcae2f7e179db837e400b5a52b969478`），從乾淨 v1.00 EXE 套用後與目標 EXE byte-identical；最新發布 zip SHA-256 `04239db94ee15013a98de9022153772b287967734f8a936e143d80091cf06a25`。前一版 EXE 備份為 `scratchpad/krondor_pre_teleport_layout.exe`。
- **近距離地圖新增可點擊、會旋轉且有鎖定狀態配色的指北針，已實機驗收定案**（2026-09-04）：指北針位於地圖 viewport 左上角；未鎖定時以白色顯示並隨地圖方向旋轉，鎖定北方時改為亮紅色並固定朝上。點擊 26×26 熱區可在鎖定／不鎖定之間切換，與既有 `N` 鍵共用同一切換函式，滑鼠按住期間只觸發一次。引擎提交 `f6e20a6`；Borland C++ 3.1 重編成功，`KRONDOR.EXE` **470608 bytes**、SHA-256 `9b8557b448e64e379dbd25ba27f11bf4c170ee7ff5020928edf63d0133a87a79`，兩個 OVL byte-identical，100 項測試通過。正式 bspatch **55243 bytes**（SHA-256 `6e6f25aa1a25bcc28a918af85b4419f53e257c1ef35ba2d6422558ed5677fe72`），從乾淨 v1.00 EXE 套用後與目標 EXE byte-identical；最新發布 zip SHA-256 `dc28d002c000896a2da877bbfb53fb913b89b6716a7e4e9f50092e7068934884`。前一版 EXE 備份為 `scratchpad/krondor_pre_clickable_north_compass.exe`。
- **1.02 非 CD 修正已回植至 1.00 中文引擎；五項實機驗收全數通過**（2026-09-04）：以 `V102_GAMEPLAY_FIXES` 將官方 1.02 中不依賴 MSCDEX／CD 資源的安全性、戰鬥、狀態、平衡與地圖行為帶入目前的 1.00 Floppy 中文建置；CD 音軌、`drive.cfg`、CD resource path 與 `CDEFAULT.DAT` 仍只受 `V102CD` 控制。使用者已確認：（1）在 3D 探索按 `M` 進入近距離地圖後，`N` 可正常切換固定北向／旋轉模式；（2）戰鬥流程正常；（3）章節切換正常；（4）存檔寫入範圍保護正常；（5）既有舊存檔可正常使用。EVG 640×400 native CJK POC 已保存於 `backup/pre-v102-backport-20260904`（checkpoint `666e7fe`）並從正式開發線撤除。引擎提交 `9744a8c`、`93913fa`、`1a9ef5e`；Borland 3.1 重編成功，測試版 `KRONDOR.EXE` **469824 bytes**、SHA-256 `ded5ddf261090cc6c268ce9199b50026243e6e681cf2c5aef24ef0247e3fcdac`，兩個 OVL byte-identical，100 項測試通過。前一版 EXE 備份為 `scratchpad/krondor_pre_v102_non_cd_backport.exe`。正式 bspatch 已重建為 **54810 bytes**（SHA-256 `c3c38478ceb9bee9b627fb73358292f7c017a9aec9b030f3d27eccacd8579a76`），從乾淨 v1.00 EXE 往返套用後與目標 EXE byte-identical；發布 zip 亦已重建（SHA-256 `57a8702ae5560a4ae8fc02050b2f5cc34804e6dc9e464fc7488551a2c81ce5a1`）。
- **第二章羅姆尼「黑羊酒館」熱區消失已修正並實機驗收**（2026-09-04）：`GDS6A.DAT` actor 0 的 `cKind = 15` 是章節結束劇情觸發器，不是底部離開列；先前 `TOWNSCN.C` 的兩處篩選誤將它排除，造成 Black Sheep Tavern 無法點擊且按住 `Tab` 沒有物件框。修正後該 actor 重新建立資料驅動的 `MenuEntry`，可正常顯示框線、點擊並進入酒館屍體劇情。引擎提交 `2e97009`；重編後 `KRONDOR.EXE` **468768 bytes**、SHA-256 `5f2cace7d6d31f7bbe1e6c2d8a2e4f5b1e70d5c532ba8d423e67e7096be28acb`，兩個 OVL byte-identical，100 項測試通過。修正前 EXE 備份為 `scratchpad/krondor_pre_romney_hotspot_fix.exe`；根因、資料欄位與防復發原則見 [`docs/research/contextual-hotspot-hints.md`](docs/research/contextual-hotspot-hints.md#2026-09-04第二章羅姆尼黑羊酒館熱區迴歸修正)。
- **2026-09-02 遊戲體驗改造已完成並實機驗收**：3D／城鎮場景可按住 `Tab` 顯示現行可互動物件角框，放開即清除；法術瀏覽器六學派圖示顯示已學數量，空分類顯示「尚未學習」；五種安全時機會輪替寫入 `GAMES\AutoSave.G99` 的 10 個自動存檔；玩家看見指定洞穴入口後，俯瞰地圖會持久顯示菱形記號；俯瞰地圖也已補齊 `W/S` 前後、`A/D` 轉向、`Q/E` 平移、`T` 紮營。當時已驗收的洞穴地標目錄只有 zone 1／shape 50；引擎提交 `6fcffc0`，主專案資源提交 `9b0a5f1`；100 項測試通過。該已驗收版 `KRONDOR.EXE` 466896 bytes、SHA-256 `0807073954f793b2fc3c2ad183586b883e99c5c99db07f10ec1c7bfe744dad13`。
- **地下地點目錄已排除世界地圖既有城鎮，現為 8 個地表據點，待實機確認**（2026-09-04）：使用者在克朗多附近發現菱形中心沒有入口；稽核確認該點是 `t020900` 城市模型原點，真正進城觸發帶約在 25,960 世界單位外，且下水道入口位於城內。這與功能「補充世界地圖未標示、方便日後返回尋找的地點」的原始目的不符，因此 `2c55433` 移除薩斯、克朗多、薩薩戈斯三個普通城鎮代理點，保留五處直接地下切換、兩處封閉／條件式 `entrance`，以及世界地圖未標示的瀑布洞穴。既有事件 1～7 與瀑布事件 10 不重排；存檔中舊有 8、9、11 旗標會被忽略，無須轉檔。Borland 3.1 重編完成，`KRONDOR.EXE` **468720 bytes**、SHA-256 `e9e33565e42840fcaa2b2a24c43c4d1d559134a087fdd796259d5cdef7990795`，兩個 OVL byte-identical；100 項測試通過。測試版已部署，前一版備份為 `scratchpad/krondor_pre_landmarks_no_towns.exe`。待確認薩斯、克朗多、薩薩戈斯不再出現菱形，並繼續驗收其餘新增地下據點；驗收前不更新 `engine-patches/`。詳見 [`docs/research/ux-improvements-2026-09-02.md`](docs/research/ux-improvements-2026-09-02.md)。
- **法術察覺興趣點已可留存在俯瞰地圖，首輪漏接已修正，待複驗**（2026-09-03）：`伊夏之眼`、`The Unseen`、`Nacre Cicatrix` 三種察覺法術每次真正畫出紅點時，會記錄對應世界物件；關閉法術介面後，普通俯瞰地圖仍以原版風格小紅點顯示，存檔／讀檔後也會保留。首輪實機發現同一次法術的兩個紅點只留下一筆；直接檢查當時 `TEMP.GAM` 證實只有 zone 1／shape 162／`(672800,876000)` 一筆。原因是另一筆來自遭遇角色清單，而首版刻意排除了移動／臨時目標。依「法術實際顯示即留存」規則，`0076d4d` 已補上三種法術的遭遇清單路徑。資料仍以向後相容的尾端擴充區附加在 `TEMP.GAM`／存檔，舊存檔沒有擴充區時視為零筆；上限 1600 筆。Borland 3.1 重編完成，`KRONDOR.EXE` **468624 bytes**、SHA-256 `7ffba1b175a05942d5d5963f322c01aceef68b9c5803ed55726c45e9fdbba8d4`，兩個 OVL byte-identical；100 項測試通過。測試版已部署，漏接版備份為 `scratchpad/krondor_pre_all_spell_targets_20260903.exe`。
- **紮營畫面殘留英文按鈕已改為小字中文，一般狀態配色已調亮，待複驗**（2026-09-04）：`REQ_CAMP.DAT` 的三個控制項沒有字串欄位，`Rest Until Healed`／`Cancel`／`Exit` 是直接烙在 sprite 圖片裡，因此原本的 MenuPage 翻譯管線無法擷取。`ENCAMP.C` 載入後改用同尺寸的 type-6 原生文字按鈕，顯示「休息至康復／取消／離開」；共用 Widget 路徑會以 10×10 小字置中。首版 type-0 樣式已實機確認中文顯示正常，但其配色是一般狀態深色、按下時亮色；改用 type-6 後調整為一般狀態較亮、按下狀態較暗，不影響其他選單。為避免 `menupage_free()` 把 static 字串誤當配置的 string blob 釋放，中文指標只在每次繪圖期間暫掛，畫完立即清除。引擎提交 `ed24b0d`／配色修正 `8dbeb43`；`UI_HARDCODED.json` 新增 3 筆並重建 `ZHSTAT.DAT` 為 **990 glyph／21790 bytes**。Borland 3.1 重編完成，`KRONDOR.EXE` **468768 bytes**、SHA-256 `9bc7192a9e3250a8f60c79d31a92400a584449f4ef6fdab0d7cad17753054271`，兩個 OVL byte-identical；100 項測試通過。配色修正前 EXE 備份為 `scratchpad/krondor_pre_encamp_button_colors_20260904.exe`。
...
6. 還原 WSL clone：目前 WIP 的基底是 `d702c75`，使用 `wsl -e bash -lc "cd ~/krondor-build && git reset --hard d702c75 && git stash pop"` 回到 VESA WIP 狀態；若基底日後改變，應先記錄建置前的 `git rev-parse HEAD`，再 reset 回該精確提交，不可照抄舊值。
...
8. 目前 `dist/test_v100_zh/krondor.exe` 由 Windows upstream `2c55433` 重編，**468720 bytes**，SHA-256 `e9e33565e42840fcaa2b2a24c43c4d1d559134a087fdd796259d5cdef7990795`；最新功能包含 Tab 可互動熱區、法術分類數量／空狀態、輪替自動存檔、8 個世界地圖未標示的地下地表據點範圍地標與俯瞰地圖 WASDQE/T 操作。同時部署 loose `REQ_MAIN.DAT` 與 `REQ_MAP.DAT`（兩者帳篷圖示 action_id 均為 `0x14`）。**`dist/dosbox_zh_test.conf` 的 `[dos]` 區塊（`dos=high,umb`）仍不可移除，UMB 後援需要它。**

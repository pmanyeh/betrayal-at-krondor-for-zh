...
**最後更新：** 2026-09-03（本輪遊戲體驗改造均已由使用者實機驗收：**按住 Tab 顯示可互動熱區**、**法術分類顯示已學數量／空分類顯示「尚未學習」**、**10 槽輪替自動存檔**、**地圖顯示已發現洞穴入口記號**、**俯瞰地圖支援 WASDQE 與 T 紮營**。以物件中心為基準、隨地圖視野縮放且持久留存的範圍菱形已確認正常；地標目錄已依十章切換、傳送、對話、GDS 場景與 WLD 資料完整交叉稽核，擴充為 11 個洞穴／礦坑／地城地表據點。新版另會將三種察覺法術實際顯示的所有結果持久顯示為俯瞰地圖紅點，並將紮營畫面三個圖片式英文按鈕改成 10×10 中文小字；已完成重編與離線驗證，待實機驗收。完整紀錄見 [`docs/research/ux-improvements-2026-09-02.md`](docs/research/ux-improvements-2026-09-02.md)。）
...
- **紮營畫面殘留英文按鈕已改為小字中文，待實機驗收**（2026-09-03）：`REQ_CAMP.DAT` 的三個控制項沒有字串欄位，`Rest Until Healed`／`Cancel`／`Exit` 是直接烙在 sprite 圖片裡，因此原本的 MenuPage 翻譯管線無法擷取。`ENCAMP.C` 載入後改用同尺寸的原生文字按鈕，顯示「休息至康復／取消／離開」；共用 Widget 路徑會以 10×10 小字置中。為避免 `menupage_free()` 把 static 字串誤當配置的 string blob 釋放，中文指標只在每次繪圖期間暫掛，畫完立即清除。引擎提交 `ed24b0d`；`UI_HARDCODED.json` 新增 3 筆並重建 `ZHSTAT.DAT` 為 **990 glyph／21790 bytes**。Borland 3.1 重編完成，`KRONDOR.EXE` **468768 bytes**、SHA-256 `becb8ea51d34971e4015f5c9aea87ade0a59608b120f6300e90f80f7ad3d8ec2`，兩個 OVL byte-identical；100 項測試通過。前一版 EXE／小字庫備份為 `scratchpad/krondor_pre_encamp_buttons_20260903.exe`、`scratchpad/ZHSTAT_pre_encamp_buttons_20260903.DAT`。

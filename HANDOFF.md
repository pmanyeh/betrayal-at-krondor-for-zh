...
- **法術察覺到的固定興趣點已可留存在俯瞰地圖，待實機驗收**（2026-09-03）：`伊夏之眼`、`The Unseen`、`Nacre Cicatrix` 三種察覺法術每次真正畫出紅點時，會記錄對應的固定世界物件；關閉法術介面後，普通俯瞰地圖仍以原版風格小紅點顯示，存檔／讀檔後也會保留。敵人、戰鬥遭遇等移動或臨時目標刻意不寫入。資料以向後相容的尾端擴充區附加在 `TEMP.GAM`／存檔，舊存檔沒有擴充區時視為零筆；上限 1600 筆，資料稽核的候選上界低於 1061。引擎提交 `e9887bf`＋邊界修正 `b157522`；Borland 3.1 重編完成，`KRONDOR.EXE` **468576 bytes**、SHA-256 `809420a21340fa8f7750784084556e5c83a97fc45a4e69611075e20d51f2daff`，兩個 OVL byte-identical；100 項測試通過。測試版已部署，前一版備份為 `scratchpad/krondor_pre_spell_landmarks_20260903.exe`。

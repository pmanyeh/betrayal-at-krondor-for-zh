...
**最後更新：** 2026-09-03（本輪遊戲體驗改造均已由使用者實機驗收：**按住 Tab 顯示可互動熱區**、**法術分類顯示已學數量／空分類顯示「尚未學習」**、**10 槽輪替自動存檔**、**地圖顯示已發現洞穴入口記號**、**俯瞰地圖支援 WASDQE 與 T 紮營**。以物件中心為基準、隨地圖視野縮放且持久留存的範圍菱形已確認正常；地標目錄現再擴充至戶外全部 7 個洞穴／地城山體入口，完成重編與離線驗證，待實機確認新增四處。完整紀錄見 [`docs/research/ux-improvements-2026-09-02.md`](docs/research/ux-improvements-2026-09-02.md)。）
...
- **洞穴／地城山體入口已擴充為戶外全部 7 處，待實機確認新增四處**（2026-09-03）：使用者已確認 `949ca96` 的持久留存、縮放投影版正常。進一步掃描 `Z01`～`Z09` 的所有模型與 WLD 擺放，除 zone 1／2／6 的三個 `entrance` 外，另找到 zone 5 一個、zone 8 兩個、zone 9 一個 `stonemtn`；四者均鄰近通往 zone 10／12 的地城切換熱區，故全數納入，事件編號新增 4～7，既有 1～3 不變以保持存檔相容。引擎提交 `ec9c3b4`；Borland 3.1 重編完成，`KRONDOR.EXE` **467232 bytes**、SHA-256 `8fc95a59d13a7470828686b6fb3098bebe5fe1eba1bb844015fd28e502e56d5d`，兩個 OVL byte-identical；100 項測試通過。測試版已部署，前一版備份為 `scratchpad/krondor_pre_all_entrances_20260903.exe`。新增四處驗收前不更新 `engine-patches/` 封存基準。詳見 [`docs/research/ux-improvements-2026-09-02.md`](docs/research/ux-improvements-2026-09-02.md)。
...
8. 目前 `dist/test_v100_zh/krondor.exe` 由 Windows upstream `ec9c3b4` 重編，**467232 bytes**，SHA-256 `8fc95a59d13a7470828686b6fb3098bebe5fe1eba1bb844015fd28e502e56d5d`；最新功能包含 Tab 可互動熱區、法術分類數量／空狀態、輪替自動存檔、戶外 7 處已發現洞穴／地城範圍地標與俯瞰地圖 WASDQE/T 操作。同時部署 loose `REQ_MAIN.DAT` 與 `REQ_MAP.DAT`（兩者帳篷圖示 action_id 均為 `0x14`）。**`dist/dosbox_zh_test.conf` 的 `[dos]` 區塊（`dos=high,umb`）仍不可移除，UMB 後援需要它。**

...
**最後更新：** 2026-08-27（新增隊伍六名固定角色名字的翻譯與部署——`STARTUP.GAM`／`TEMP.GAM`／全部測試存檔）
...
- **BOK 書籍系統：C11 試點已完成並實機驗收通過**（2026-08-27，原本 [text-surface-inventory.md §3](docs/research/text-surface-inventory.md) 標「未動工」的一整塊，Phase 7）。做完的三件事：
  ...
  - **C11.BOK 翻譯部署＋實機驗收**：14 段全翻（`localization/translated/BOK_C11.json`），字庫涵蓋率 0 缺字，`bok_translate.py build` 打包成 `dist/test_v100_zh/C11.BOK`（loose 檔覆蓋，`res_fopen` 先找 loose 檔再翻 RMF，跟 DDX 部署同機制）。DOSBox-X 開新遊戲跑到第一章開場書，14 段全數在 2 頁內正確顯示：整段內文齊行、短對話行靠左、段距、`——`／`……`／`「」`／`，` 全形標點、翻頁、退出書本接 TTM 過場都正常。
  - **已知 & 待辦**（見下方「其他待辦」）：(1) 首字放大圖目前仍是英文燙金「B」（互動式暫留，等 BMX codec）；(2) CJK 行首標點（`。」？`）未做避頭尾，跟 DDX 那條「孤兒行／孤立標點」是同一個引擎層限制，非迴歸；(3) 其餘 21 個 BOK 檔還沒翻。
...
6. **BOK 剩下 21 個章節書**（`C12`／`C21`/`C23`/`C31`/`C32`/`C41`/`C43`~`C46`/`C51`~`C53`/`C61`/`C63`/`C71`/`C81`/`C83`/`C91`/`C92`/`C94`）：codec 跟引擎都已就緒，剩純翻譯。流程：`python tools/text/bok_extract_pristine.py`（22 個都挖好在 `scratchpad/pristine_bok/`）→ `bok_translate.py scaffold scratchpad/pristine_bok/Cxx.BOK` → 填 `localization/translated/BOK_Cxx.json` → `bok_translate.py build ... dist/test_v100_zh/Cxx.BOK`。建議比照 DDX 寫一支 `bok_rebuild_all.py`＋manifest 做 all-or-nothing 批次重建，並在 build 前跑字庫涵蓋率掃描（`C94` 最大，13 KB／54 段，其餘多在 1~3 KB／4~21 段）。專有名詞照 `glossary.json`。
7. **BOK 首字放大圖改中文（設計決定已拍板：重繪成中文首字點陣圖）**：需要新寫 `BOOK.BMX` 影像 codec（IFF `BMP:INF:/BIN:/VGA:/AMG:` chunk＋BAK 專有壓縮，可參考 xbak/OpenBAK 文件），解出全部 19 張圖、只重編 image 0，用 `build_font.py` 的倚天點陣器把該章中文首字放大到約 72×80 套 `BOOK.PAL` 顏色回填；`bok_pack.py` 對應把該段文字開頭那個字移除（原文就是靠圖補首字母）。這是整個 BOK 工作剩下最大的單一新元件，排在 21 章翻譯之後或平行處理都行。C11 的中文首字建議用「血」。

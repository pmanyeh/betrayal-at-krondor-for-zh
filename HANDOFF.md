...
**最後更新：** 2026-09-03（本輪遊戲體驗改造均已由使用者實機驗收：**按住 Tab 顯示可互動熱區**、**法術分類顯示已學數量／空分類顯示「尚未學習」**、**10 槽輪替自動存檔**、**地圖顯示已發現洞穴入口記號**、**俯瞰地圖支援 WASDQE 與 T 紮營**。洞穴地標已擴充 zone 2／6，並改為以物件中心為基準、隨地圖視野縮放的範圍菱形；完成重編與離線驗證，待實機驗收。完整紀錄見 [`docs/research/ux-improvements-2026-09-02.md`](docs/research/ux-improvements-2026-09-02.md)。）
...
- **洞穴地標擴充及範圍標記版，待實機複驗**（2026-09-03）：RMF 掃描確認戶外 `entrance` shape 50 僅出現在 zone 1／2／6，事件編號為 1／2／3。標記直接置於物件原點；大型空心菱形半徑取模型 `Shape.radius << Shape.shift`，再依地圖 zoom／相機高度換算為畫面尺寸並限制在 7～36 像素。首版 `90091ea` 實機發現離遠與切換縮放後會消失；`949ca96` 已移除固定 300000 距離裁切、改用原版偵測法術紅點的純俯瞰投影，並允許菱形在地圖邊緣局部裁切顯示。Borland 3.1 重編完成，`KRONDOR.EXE` **467184 bytes**、SHA-256 `4a5de1ab1f182b7b4374e5b1c1dbe9989182d202719aaef8dc37ff92ad5fd629`，兩個 OVL byte-identical；100 項測試通過。Git 回復點為兩個 repo 的 `backup/pre-landmark-range-marker-20260903`。測試版已部署；前一版備份為 `scratchpad/krondor_pre_landmark_persistence_fix_20260903.exe`。待複驗遠離後仍顯示、各縮放層級均顯示、地圖邊緣裁切及存檔持久性，驗收前不更新 `engine-patches/` 封存基準。詳見 [`docs/research/ux-improvements-2026-09-02.md`](docs/research/ux-improvements-2026-09-02.md)。
...
8. 目前 `dist/test_v100_zh/krondor.exe` 由 Windows upstream `949ca96` 重編，**467184 bytes**，SHA-256 `4a5de1ab1f182b7b4374e5b1c1dbe9989182d202719aaef8dc37ff92ad5fd629`；最新功能包含 Tab 可互動熱區、法術分類數量／空狀態、輪替自動存檔、已發現洞穴範圍地標與俯瞰地圖 WASDQE/T 操作。同時部署 loose `REQ_MAIN.DAT` 與 `REQ_MAP.DAT`（兩者帳篷圖示 action_id 均為 `0x14`）。**`dist/dosbox_zh_test.conf` 的 `[dos]` 區塊（`dos=high,umb`）仍不可移除，UMB 後援需要它。**

...
- **新手輕鬆開局存檔**：使用者原本想把「別人提供的」`SAVE05.GAM`（改名 `NewGamePlus.G01/SAVE01.GAM`）直接收進發布包，經確認這份存檔不是本專案自己的內容、來源授權不明，改為自建（`tools/release/build_starter_save.py`）。**第一版**從乾淨的 `STARTUP.GAM` 直接改起始金幣＋套用姓名 patch，結果使用者實機一讀取就閃退（`cannot load gi block` / `Null pointer assignment`，對應 `SRC/R3D/SCENE/PROXIM.C: proximity_table_load` 的 `panic()`）——查 `GMAIN.C: gmain_start_dispatch()` 發現 New Game（mode 2，讀 `STARTUP.GAM`）跟 Load Game（mode 3，讀 `GAMES\...\SAVE0N.GAM`）雖然都走同一個 `savegame_read()`，但 `STARTUP.GAM` 在原版遊戲裡**從來沒有被 Load Game 這條路徑讀過**，顯然有某些欄位（很可能是 `shared_inventory`/`ground_pile` 那兩個 far pointer 相關的區域）只有 New Game 流程會補做初始化、Load Game 流程預期存檔裡已經是可用狀態。**第二版改成不用 `STARTUP.GAM`**，改用本專案自己過去測試留下、已知能正常 Load Game 讀取的真實存檔當基準（`tools/release/starter_save_base/verified_chapter1_save.GAM`，來源 `dist/test_v100_zh/GAMES/SAVES.G01/SAVE01.GAM`，章節 1、原本金幣 143，名字早就是中文——這批 40 個存檔本來就是本專案自己測試時的遊玩紀錄，跟「別人提供」的授權疑慮無關），只改金幣欄位（143 → 1000），其餘一律原封不動。金幣 offset（file offset 102、4-byte signed LE）推導與交叉驗證方式不變，見指令碼 docstring。版控例外 `.gitignore` 新增 `!tools/release/starter_save_base/**/*.GAM`。**這一版還沒實機重新驗證過**（session 當下 DOSBox-X AI bridge 沒有連線中的遊戲程序）——下次有機會請優先確認「讀取進度→NewGamePlus」不再閃退、金幣顯示 1000。
- 同一時間，使用者自己修好 `patch_character_names.py` 兩個問題：(1) `zh_mapping.json` 原本用 cwd 相對路徑找，從 `tools/text/` 以外的目錄執行會 `FileNotFoundError`，改用 `__file__` 推導 repo root；(2) 外來存檔（例如上面提到的 `SAVE05.GAM`）的姓名欄位在 NUL 結尾後常留有非零 stale bytes 而非全零 padding，原本嚴格驗證會拒絕，現在改成只要求「英文名＋NUL」開頭就接受、安全覆寫整個 10-byte slot。已用該外來存檔驗證：長度不變、只有姓名欄位改變。
- **新手存檔擴充為兩個存檔點、資料夾改名 `Plus.G01`**：使用者自己另外實際遊玩產生一份章節 1 進度存檔（已確認是自己玩出來的，不是別人提供，沒有授權疑慮），想跟原本的「金幣調高版」一起收進發布包。`build_starter_save.py` 改為輸出兩個檔案到同一個 `GAMES\Plus.G01\` 資料夾（比照遊戲原生的多存檔點慣例，`SAVE01.GAM`／`SAVE02.GAM`）：`SAVE01.GAM` 沿用先前驗證過的金幣調整邏輯（143→1000，來源 `tools/release/starter_save_base/verified_chapter1_save.GAM`）；`SAVE02.GAM` 只套用姓名中文化 patch（來源 `tools/release/starter_save_base/own_playthrough_chapter1_save.GAM`，原始姓名欄位是英文＋NUL 後 stale bytes，正好用得上使用者剛修好的 `patch_character_names.py` 容錯邏輯），金幣與進度完全保留玩家原樣。`package_release.py` 的 `write_starter_save()` 改成整個 slot 資料夾當單位複製（`STARTER_SAVE_SLOT_RELPATH`），`check_game_data_is_empty()` 白名單邏輯同步改成認整個資料夾而非單一檔名，安裝說明／README 文字同步更新成「Plus」＋兩個存檔點的說明。同樣尚未實機驗證。
- 已在乾淨複本上完整跑過：解壓整合包＋丟入原版遊戲檔案＋雙擊「安裝中文化.bat」＋雙擊「玩遊戲.bat」（`dosbox-x.exe` 正常啟動、無警告）＋`--uninstall` 還原，EXE／GAM／74 個資源檔都跟 `dist/test_v100_zh/` 位元組一致。**這只是工具鏈就緒，不代表翻譯本身已達可公開發布狀態**——下方「待處理」清單的實機驗收/校對項目仍待完成。詳見 `tools/release/` 內各檔案 docstring 與 `README.md`「如何取得中文化版本」一節。先前（2026-08-30）：法術三檔／戰鬥面板／`MNAMES.DAT`／`fmap_twn.dat` 見下；**新增遊戲選單 `req_*.dat`（MenuPage 家族）**——通用 `menupage_translate.py` codec，翻 11 個玩家會看到的選單畫面（主選單／存讀檔／偏好設定／治療／物品欄／傳送清單），約 45 個場景編輯器／作弊 `req_*` 檔掃描但不翻；`WIDGET.C` 按鈕標籤改小字。`upstream dbf3288`、`krondor.exe` 458816 bytes（`MODALSCR.C` 旅店/休息畫面「隊伍金幣」行也改小字）。主線翻譯 checkpoint：`86f9d93`／`adf87e0`／`18e48ec`。主選單已實機確認中文＋不閃退。其餘皆待完整實機驗收）。先前：GoodBye 修正與 Ask About 選項翻譯／小字型已實機驗收。

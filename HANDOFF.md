...
**最後更新：** 2026-09-04（本輪遊戲體驗改造均已由使用者實機驗收：**按住 Tab 顯示可互動熱區**、**法術分類顯示已學數量／空分類顯示「尚未學習」**、**10 槽輪替自動存檔**、**地圖顯示已發現洞穴入口記號**、**俯瞰地圖支援 WASDQE 與 T 紮營**。以物件中心為基準、隨地圖視野縮放且持久留存的範圍菱形已確認正常；地標目錄已依十章切換、傳送、對話、GDS 場景與 WLD 資料完整交叉稽核，擴充為 11 個洞穴／礦坑／地城地表據點。新版另會將三種察覺法術實際顯示的所有結果持久顯示為俯瞰地圖紅點；紮營畫面三個英文圖片按鈕也已改為 10×10 中文小字，並依實機回報將一般狀態改為較亮、按下狀態改為較暗；已完成重編與離線驗證，待複驗配色。完整紀錄見 [`docs/research/ux-improvements-2026-09-02.md`](docs/research/ux-improvements-2026-09-02.md)。）
...
- **洞穴／礦坑／地城入口已擴充為 11 個地表據點，待實機確認新增據點**（2026-09-03）：使用者已確認 `949ca96` 的持久留存、縮放投影版正常。十章戶外 hotspot、`DEF_ZONE.DAT`、`TELEPORT.DAT`、入口對話、GDS 場景與 WLD 擺放已完成交叉稽核：五處為直接地下切換，兩處為封閉／條件式 `entrance` 物件，另四處是經由薩斯、克朗多、瀑布與薩薩戈斯 2D 場景抵達地下區域的地表據點。薩斯及克朗多各有兩個室內入口，但俯瞰地圖上各合併為同一據點；純劇情俘虜傳送則排除。既有事件 1～7 不變，新據點使用 8～11，以保持存檔相容。引擎提交 `2648f99`；Borland 3.1 重編完成，`KRONDOR.EXE` **467280 bytes**、SHA-256 `de9670797f12432b2abca4f7c98cd011243e5eb371039e4da746bee5fd16452c`，兩個 OVL byte-identical；100 項測試通過。測試版已部署，前一版備份為 `scratchpad/krondor_pre_all_underground_sites_20260903.exe`。新據點驗收前不更新 `engine-patches/` 封存基準。詳見 [`docs/research/ux-improvements-2026-09-02.md`](docs/research/ux-improvements-2026-09-02.md)。

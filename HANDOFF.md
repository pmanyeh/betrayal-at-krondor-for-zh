...
**最後更新：** 2026-08-31（**新增發布用補丁工具鏈** `tools/release/`：`build_exe_patch.py` 用 `bsdiff4` 對 v1.00 baseline EXE 產生二進位差異補丁；`installer.py`（純標準函式庫、免 pip install，內嵌 `bspatch_apply.py` 這支自寫的 BSDIFF4 patch-apply）給使用者對自己合法版本的遊戲套用；`package_release.py` 從 `dist/test_v100_zh/` 的 `*_BUILD_MANIFEST.json` 抓已翻譯資源檔清單組出 `dist/release_v100_zh/`（EXE 補丁＋`STARTUP.GAM` 角色名 patch data＋74 個 loose 資源檔＋安裝說明），全程不散布任何原版資產或編譯好的 EXE。已在乾淨測試複本上跑過完整安裝／解除安裝，EXE／GAM／全部資源檔都跟 `dist/test_v100_zh/` 位元組一致；EXE 版本不符時會安全中止不留殘檔。頂層新增 `LICENSE`（比照 upstream 寫法）。**這只是工具鏈就緒，不代表翻譯本身已達可公開發布狀態**——下方「待處理」清單的實機驗收/校對項目仍待完成。詳見 `tools/release/` 內各檔案 docstring 與 `README.md`「如何取得中文化版本」一節。先前（2026-08-30）：法術三檔／戰鬥面板／`MNAMES.DAT`／`fmap_twn.dat` 見下；**新增遊戲選單 `req_*.dat`（MenuPage 家族）**——通用 `menupage_translate.py` codec，翻 11 個玩家會看到的選單畫面（主選單／存讀檔／偏好設定／治療／物品欄／傳送清單），約 45 個場景編輯器／作弊 `req_*` 檔掃描但不翻；`WIDGET.C` 按鈕標籤改小字。`upstream dbf3288`、`krondor.exe` 458816 bytes（`MODALSCR.C` 旅店/休息畫面「隊伍金幣」行也改小字）。主線翻譯 checkpoint：`86f9d93`／`adf87e0`／`18e48ec`。主選單已實機確認中文＋不閃退。其餘皆待完整實機驗收）。先前：GoodBye 修正與 Ask About 選項翻譯／小字型已實機驗收。

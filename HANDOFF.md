...
- **1.02 非 CD 修正已回植至 1.00 中文引擎；地圖、戰鬥與章節切換已實機驗收**（2026-09-04）：以 `V102_GAMEPLAY_FIXES` 將官方 1.02 中不依賴 MSCDEX／CD 資源的安全性、戰鬥、狀態、平衡與地圖行為帶入目前的 1.00 Floppy 中文建置；CD 音軌、`drive.cfg`、CD resource path 與 `CDEFAULT.DAT` 仍只受 `V102CD` 控制。使用者已確認：（1）在 3D 探索按 `M` 進入近距離地圖後，`N` 可正常切換固定北向／旋轉模式；（2）戰鬥流程正常；（3）章節切換正常。EVG 640×400 native CJK POC 已保存於 `backup/pre-v102-backport-20260904`（checkpoint `666e7fe`）並從正式開發線撤除。引擎提交 `9744a8c`、`93913fa`、`1a9ef5e`；Borland 3.1 重編成功，測試版 `KRONDOR.EXE` **469824 bytes**、SHA-256 `ded5ddf261090cc6c268ce9199b50026243e6e681cf2c5aef24ef0247e3fcdac`，兩個 OVL byte-identical，100 項測試通過。前一版 EXE 備份為 `scratchpad/krondor_pre_v102_non_cd_backport.exe`。存檔寫入保護與既有存檔相容性仍待實機驗收，驗收前不更新正式 bspatch／發布包。

...
- **洞穴地標擴充及範圍標記版，待實機驗收**（2026-09-03）：RMF 掃描確認戶外 `entrance` shape 50 僅出現在 zone 1／2／6，事件編號為 1／2／3。依使用者新版設計，標記不再手工偏移至入口立面，而是直接置於物件原點；大型空心菱形半徑取模型 `Shape.radius << Shape.shift`，再依地圖 zoom／投影深度換算為畫面尺寸並限制在 7～36 像素，因此會隨視野縮放。引擎提交 `90091ea`；Borland 3.1 重編完成，`KRONDOR.EXE` **467136 bytes**、SHA-256 `c949d2d19656e2db0dde95fe807921d37ba086eb0e689ec4664f3920818fe478`，兩個 OVL byte-identical；100 項測試通過。Git 回復點為兩個 repo 的 `backup/pre-landmark-range-marker-20260903`。測試版已部署，舊 EXE 備份為 `scratchpad/krondor_pre_landmark_range_20260903.exe`；待驗收置中、涵蓋範圍、縮放反應與存檔持久性，驗收前不更新 `engine-patches/` 封存基準。詳見 [`docs/research/ux-improvements-2026-09-02.md`](docs/research/ux-improvements-2026-09-02.md)。

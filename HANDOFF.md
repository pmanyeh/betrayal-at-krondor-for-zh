...
- **新手輕鬆開局存檔**：使用者原本想把「別人提供的」`SAVE05.GAM`（改名 `NewGamePlus.G01/SAVE01.GAM`）直接收進發布包，經確認這份存檔不是本專案自己的內容、來源授權不明，改為**完全從乾淨的 `STARTUP.GAM` 自建**（`tools/release/build_starter_save.py`）：只調整起始金幣（0 → 1000，`GameState.nParty_gold` 位於存檔 file offset 102、4-byte signed LE——offset 是從 `SAVEGAME.C` 的 100-byte header 結構＋`GMAIN.H` 的 `GameState` 欄位順序推導，並拿 `dist/test_v100_zh/GAMES/` 底下 40 個既有實測存檔的金幣數值反向交叉驗證過，逐存檔數值都隨進度合理遞增，確認 offset 沒抓錯）＋套用既有的隊伍姓名中文化 patch，其餘章節/能力/物品完全不動。輸出到 `tools/release/starter_save/GAMES/NewGamePlus.G01/SAVE00.GAM`（版控例外：`.gitignore` 新增 `!tools/release/starter_save/**/*.GAM`，因為這是本專案自己產出的內容，不算原版資產），`package_release.py` 打包時連同「把遊戲資料放這裡.txt」一起收進 `game_data/`，白名單安全檢查也同步更新成認得這兩個檔案。**尚未在實機／debugger 掛上跑過「讀取進度→NewGamePlus」這個流程**（session 當下 DOSBox-X AI bridge 沒有連線中的遊戲程序），僅有離線位元組層級驗證；下次有機會建議實機點一次確認讀取畫面顯示正常、金幣確實是 1000。
